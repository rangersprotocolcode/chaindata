...
func (self *ethModule) processEvent(i int64, blockhash, ts string, list []common.Address, client *ethclient.Client) {
	logs := self.getLogs(i, list, client)
	if nil == logs {
		return
	}
	...
	for _, log := range logs {
		...
		mysql.InsertLogs(i, self.chainId, blockhash, ts, log.TxHash.String(), fromAddr.String(), toAddr.String(), value.String(), log.Address.String())
	}

}

func (self *ethModule) getLogs(i int64, list []common.Address, client *ethclient.Client) []types.Log {
	var (
		logs []types.Log
		err  error
	)
